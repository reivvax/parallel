...
typedef struct Monitor {
    ...
    _Atomic int stack_size;         // keep atomic stack size so you can have Stack's size not atomic
...
static void take_work(Monitor* m, Stack* dst, int id) {
    ...
    if (empty(&m->s) || m->signalling || m->waiting_for_work > 0) { // No work to take
        ...
        if (m->s.size > MONITOR_THRESHOLD) {
            int count = m->s.size / 2;
            rearrange_n(&m->s, dst, count);
            m->stack_size -= count;
        } else {
...
static void share_work(Monitor* m, Node* top, Node* bottom, int elems_count, int id) {
    ASSERT_ZERO(pthread_mutex_lock(&m->mutex));
    ...
    push_n(&m->s, top, bottom, elems_count);
    m->stack_size += elems_count;
...
// static void share_work(Monitor* m, Stack* src, int id) {
//     ASSERT_ZERO(pthread_mutex_lock(&m->mutex));
//     m->waiting_to_give_work++;
//     while (m->signalling) {
//         LOG("%d: I CANNOT ENTER SO SOON, FIRST TAKERS", id);
